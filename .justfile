...
build-local: binding
    if [ -f build/CMakeCache.txt ]; then \
      cache_src="$(sed -n 's#^CMAKE_HOME_DIRECTORY:INTERNAL=##p' build/CMakeCache.txt | head -n1)"; \
      if [ -n "$cache_src" ] && [ "$cache_src" != "$PWD" ]; then \
        echo "Resetting stale build cache from $cache_src"; \
        rm -rf build; \
      fi; \
    fi
