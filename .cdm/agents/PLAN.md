...
- [ ] `cdm --quiet` runs without any stdout/stderr output on success
- [ ] `cdm --quiet` shows error messages on failure
- [ ] `cdm --quiet --verbose` errors with conflict message
...
```bash
cargo build && cargo clippy && cargo test
cdm --quiet --step  # Should be silent on success
cdm --quiet --step 2>&1 | wc -l  # Should be 0 lines on success
```
```
